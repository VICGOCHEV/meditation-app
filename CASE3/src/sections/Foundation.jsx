...
// поштучное появление сватчей: каждый «вспыхивает» из лёгкого blur+scale
const swatchVar = {
  hidden: { opacity: 0, scale: 0.82, y: 12, filter: 'blur(6px)' },
  show: { opacity: 1, scale: 1, y: 0, filter: 'blur(0px)', transition: { duration: 0.5, ease: [0.22, 0.9, 0.3, 1] } },
}
const swatchGrid = { hidden: {}, show: { transition: { staggerChildren: 0.06, delayChildren: 0.08 } } }

function Swatch({ bg, tag, name, val, dark }) {
  return (
    <motion.div variants={swatchVar}>
        ...
        <Panel>
          {TYPE.map((t, i) => (
            <div key={t.l} className="grid grid-cols-1 items-center gap-3 py-5 sm:grid-cols-[90px_1fr_auto] sm:gap-6" style={i ? { borderTop: '1px solid rgba(180,160,255,0.045)' } : undefined}>
              ...
              <Typewriter text={t.s} startDelay={i * 260} speed={t.s.length > 50 ? 12 : 26} className={t.mono ? 'mono' : ''} style={{ color: '#f4f0ff', ...t.style }} />
