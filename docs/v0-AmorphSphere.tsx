...
/**
 * AmorphSphere — фирменный шейдер бренда Meditation.
 * Drop-in компонент для V0.dev / Next.js 14 (App Router).
 *
 * Зависимости (yarn add ...):
 ...
 *
 * ВАЖНО:
 * - Компонент абсолютно позиционируется (inset-0 h-full w-full).
 *   Родитель должен иметь position:relative и заданные размеры.
 * - Без mix-blend-mode по умолчанию — шейдер уже выдаёт корректный
 *   RGBA, прозрачность за пределами «капли» сохраняется.
 * - На мобильных тяжело: dpr clamp до [1, 2], geometry — fullscreen
 *   plane (без 3D-сетки), ~60fps на iPhone 12+.
 */
...
interface AmorphSphereProps {
  className?: string
  /** Если нужно screen-blend поверх тёмного фона — передай 'screen' */
  blendMode?: 'screen' | 'lighten' | 'normal'
}

export default function AmorphSphere({ className, blendMode }: AmorphSphereProps) {
  return (
    <div
      className={className ?? 'absolute inset-0 h-full w-full'}
      style={blendMode && blendMode !== 'normal' ? { mixBlendMode: blendMode } : undefined}
    >
      <Canvas
        gl={{
          antialias: true,
